...
test('Test getSpotPrice structure', async () => {
  ...
  price = await api.hydraDx.query.getSpotPrice(asset1.toString(), asset2.toString(), '500');
  expect(price.toString()).toBe('0');

  price = await api.hydraDx.query.getSpotPrice(asset1.toString(), (asset2 + 1).toString(), '500');
  expect(price.toString()).toBe('0');

  price = await api.hydraDx.query.getSpotPrice((asset2 + 1).toString(), (asset2 + 2).toString(), '500');
  expect(price.toString()).toBe('0');

  price = await api.hydraDx.query.getSpotPrice((asset2 + 2).toString(), (asset2 + 1).toString(), '500');
  expect(price.toString()).toBe('0');
});
